function [T, W, name, Wtype, id] = intra_period(T, type, subjects, period, trials, SYN, N, toPlot)
...
for i_sub = subjects
    ...
    for i_trial = trials
        ...
        for i_period = 1:length(period)
            ...
            [id,Wtype,Wmean, Wstd] = kmeanW1122(W,N,0.6);
            T(i_sub).Synergy(i_trial).(period{i_period}).Wtype = Wtype;

            T(i_sub).Synergy(i_trial).(period{i_period}).Wmean = Wmean;
            T(i_sub).Synergy(i_trial).(period{i_period}).Wstd = Wstd;

            T(i_sub).Synergy(i_trial).(period{i_period}).id = id;
